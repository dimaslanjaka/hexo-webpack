...
function parseTableRow(source: string, parse: MarkdownToJSX.NestedParser, state: MarkdownToJSX.State) {
...
function parseTableCells(source: string, parse: MarkdownToJSX.NestedParser, state: MarkdownToJSX.State) {
...
function parseTable(capture: RegExpMatchArray, parse: MarkdownToJSX.NestedParser, state: MarkdownToJSX.State) {
  state._inline = true;
...
function attributeValueToJSXPropValue(key: keyof React.AllHTMLAttributes<Element>, value: string): any {
  if (key === 'style') {
...
function parserFor(
...
): (source: string, state: MarkdownToJSX.State) => ReturnType<MarkdownToJSX.NestedParser> {
  ...
  ruleList.sort(function (typeA, typeB) {
    const orderA = rules[typeA]._order;
    const orderB = rules[typeB]._order;
  ...
  function nestedParse(source: string, state: MarkdownToJSX.State): MarkdownToJSX.ParserResult[] {
    const result = [];
...
function parseBlock(parse, content, state: MarkdownToJSX.State): MarkdownToJSX.ParserResult {
  state._inline = false;
...
export function compiler(markdown: string, options: MarkdownToJSX.Options = {}) {
  ...
  function compile(input: string): JSX.Element {
    ...
    const arr = emitter(
      parser(_inline ? input : `${input.trimEnd().replace(TRIM_STARTING_NEWLINES, '')}\n\n`, {
        _inline
      })
    );
    ...
    return attributes.reduce(function (map, raw, index) {
      ...
      if (delimiterIdx !== -1) {
        ...
        if (
          ...
          (HTML_BLOCK_ELEMENT_R.test(normalizedValue) || HTML_SELF_CLOSING_ELEMENT_R.test(normalizedValue))
        ) {
  ...
  const rules: MarkdownToJSX.Rules = {
    blockQuote: {
      ...
      _parse(capture, parse, state) {
        return {
          _content: parse(capture[0].replace(BLOCKQUOTE_TRIM_LEFT_MULTILINE_R, ''), state)
        };
      },
      _react(node, output, state) {
        return <blockquote key={state._key}>{output(node._content, state)}</blockquote>;
      }
    ...
    codeBlock: {
      ...
      _react(node, output, state) {
        return (
          <pre key={state._key}>
            <code {...node._attrs} className={node._lang ? `lang-${node._lang}` : ''}>
              {node._content}
            </code>
          </pre>
        );
    ...
    gfmTask: {
      ...
      _react(node, output, state) {
        return <input checked={node._completed} key={state._key} readOnly type="checkbox" />;
      }
    ...
    heading: {
      ...
      _react(node, output, state) {
        return h(`h${node._level}`, { id: node._id, key: state._key }, output(node._content, state));
      }
    } as MarkdownToJSX.Rule<{
    ...
    link: {
      ...
      _react(node, output, state) {
        return (
          <a key={state._key} href={sanitizeUrl(node._target)} title={node._title}>
    ...
    refLink: {
      ...
      _parse(capture, parse, state) {
        return {
          ...
          _fallbackContent: parse(capture[0].replace(SQUARE_BRACKETS_R, '\\$1'), state),
          _ref: capture[2]
        };
      },
      _react(node, output, state) {
        return refs[node._ref] ? (
          <a key={state._key} href={sanitizeUrl(refs[node._ref]._target)} title={refs[node._ref]._title}>
            {output(node._content, state)}
          </a>
        ) : (
...
const Markdown: React.FC<{
  ...
  return React.cloneElement(compiler(children, options), props as JSX.IntrinsicAttributes);
};
